(function ($) {
    ...
    $(".kc-auto-fill").on('click', function () {
        var self = $(this),
            ...
            schema_id = wrapper.attr('id').replace("_schema_", ""),
            post_id = $('#post_ID').val(),
            data = {
                schema_id: schema_id,
                post_id: post_id,
        ...
        wrapper.find("> .field-container ").each(function () {
            if ($(this).attr("id") !== schema_id_full + "_active-container") {
                var item = $(this).clone(),
                    ...
                    field_container = item.find(".field-container"),
                ...
                if (typeof field === 'object' && field.length > 0) {
                    field.each(function () {
                        ...
                        if (name) {
                            var field_name = name.replace(schema_id_full, schema_id_full + "_multiple[" + count + "]");
                            // Filed Name
                            $(this).attr("id", field_name );
                            $(this).attr("name", field_name );
                            field_container = $(this).closest(".field-container");
                           var label = field_container.find("label.field-label");
                            label.attr("for", field_name );
                        }
                    });
                } else {
                    var name = field.attr("name") || '';
                    var field_name = name.replace(schema_id_full, schema_id_full + "_multiple[" + count + "]");
                    field.attr("id", field_name);
