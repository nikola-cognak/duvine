...
// If active pro version then org version not necessary.
if ( function_exists( 'kcseo' ) ) {
	deactivate_plugins( 'wp-seo-structured-data-schema/wp-seo-structured-data-schema.php' );
	return;
}

// Define KCSEO_VERSION VERSION.
if ( ! defined( 'KCSEO_VERSION' ) ) {
	define( 'KCSEO_VERSION', '1.4.12' );
}
...
if ( ! function_exists( 'KCSEO_uninstall' ) ) {
	function KCSEO_uninstall() {
		...
		$pt       = ! empty( $settings['post-type'] ) ? $settings['post-type'] : [ 'post', 'page' ];
