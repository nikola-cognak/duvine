			...
			add_action( 'admin_notices', function () { 
				...
				<div class="notice notice-info is-dismissible" data-kcseodismissable="kcseo_migration_dismiss" style="padding-top: 25px; padding-bottom: 22px;"> 
					...
					<p style="margin:0 0 2px;">
						<?php esc_html_e("We've updated some schema code with latest Google structure, That's why we need to migrate some field with the latest plugin.", 'wp-seo-structured-data-schema-pro'); ?>
					</p> 
					<p style="margin:0;">
						...
						$(function () { 
							$(document).on("click", 'div[data-kcseodismissable] .migration-button', function(e) { 
								...
								$.post(ajaxurl, {
									...
									if ( status ) {  
										$(e.target).closest('.is-dismissible').find('h3').html('<?php esc_html_e("Successfully migrated :)", 'wp-seo-structured-data-schema-pro'); ?>');
										$(e.target).remove();
									}
								}); 
							});
