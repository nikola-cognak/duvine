...
if ( ! class_exists( 'KcSeoMetaData' ) ) :
	...
	class KcSeoMetaData {
		...
		function __construct() {
			...
			add_action( 'save_post', [ $this, 'save_KcSeo_schema_data' ], 10, 3 );
		...
		function meta_box_wp_schema( $post ) {
			...
			foreach ( $schemaFields as $schemaID => $schema ) {
				...
				foreach ( $schema['fields'] as $fieldId => $data ) {
					...
					if ( in_array( $fieldId, [ 'publisher', 'publisherImage' ] ) && ! $data['value'] ) {
					...
					if ( $data['type'] === 'group' && ! empty( $data['fields'] ) ) {
						$groupMetaData = isset( $metaData[ $fieldId ] ) && ! empty( $metaData[ $fieldId ] ) ? $metaData[ $fieldId ] : [ [] ];
						$html_g        = null;
						$i             = 0;
						foreach ( $groupMetaData as $imDataId => $mData ) {
							...
							$html_g .= sprintf(
								'<div class="kcseo-group-item" data-index="%d" id="%s">%s%s%s</div>',
								$imDataId,
						...
						$htmlCont .= sprintf(
							'<div class="field-container kcseo-group-wrapper" data-duplicate="%d" data-group-id="%s" id="%s">%s</div>',
					...
					foreach ( $multipleMetaData as $mid => $mMetaDatum ) {
						...
						foreach ( $schema['fields'] as $fieldId => $data ) {
							if ( $fieldId !== 'active' ) {
								...
								if ( $data['type'] === 'group' && ! empty( $data['fields'] ) ) {
									$mGroupMetaData = isset( $mMetaDatum[ $fieldId ] ) && ! empty( $mMetaDatum[ $fieldId ] ) ? $mMetaDatum[ $fieldId ] : [ [] ];
									$html_g         = null;
									...
									$multipleHtml .= sprintf(
										'<div class="field-container kcseo-group-wrapper" data-duplicate="%s" data-group-id="%s" id="%s">%s</div>',
										isset( $data['duplicate'] ) ? true : false,
